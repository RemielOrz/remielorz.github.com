#================================================================
# File Name: onekey2.sh
# Author: LookBack
# mail: admin@dwhd.org
# Created Time: Fri May 24 15:57:04 2013
...
exit_scrip() {
		...
		*)
		...
		then
			...
			rm -rf /root/*.sh&&clear&&source ~/.bashrc&& rm -rf /root/os.txt
		fi
}
...
install_deluge135() {
		...
		then
			...
			clear&&$ONEKEY_ONLINE_INSTALL -y python-pip
			pip install flexget
...
install_rtorrent() {
		...
		$ONEKEY_ONLINE_INSTALL zip rar -y
...
install_transmission() {
		...
		trpasswd=`date +%s | sha256sum | base64 | head -c 8`
		if [ "$1" != "" ]
			then trpasswd=$1
		fi
		sudo useradd $trname
		...
		clear&&echo "${CQUESTION}你是否希望安装Flexget，若安装请输入Y，不安装请输入任意键。$CEND"
		read flexget
		clear&&echo "${CQUESTION}你是否希望安装Transmission的中文WEBui？ 若希望安装请输入 Y ，若不安装请输入任意键$CEND"
		read transmissionzh
...
EOF
		...
		then
			...
			then
				clear && cd $trdir && mv $trdir/web/index.html $trdir/web/index.original.html
...
#================================================================
	...
	$ONEKEY_ONLINE_INSTALL curl dos2unix vim sl linuxlogo -y
	clear &&sl&& cat << EOF
